...
| i18n               | `nestjs-i18n`: `lang` header → `Accept-Language` → `en`                                        |
...
pnpm dev                     # http://localhost:3000/api/v1, Swagger at http://localhost:3000/docs
```
...
| `SUPPORTED_LANGS`                | `en`          | Comma-separated; `en` is always included. Each must have a `src/i18n/<lang>/` folder, or startup fails         |
...
- Build the image with `docker build -t app .` and run it with `docker run --env-file .env -p 3000:3000 app`.
